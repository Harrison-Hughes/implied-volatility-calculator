...
class ImpliedVolatilityCalculator:
    ...
    def __init__(self, system_arguments):
        ...
        # sets 'lines to run' property as -1 if another value is not given
    ...
    def run_application(self):
        start_time = time.time()  # timer to test efficiency
        # creates a CSVFileData instance with the input file
        input_CSV_data = self.read_input_file()
        ...
        # writes the calculated data to the output file
        self.write_output_file(output_CSV_data)
    ...
    def write_output_file(self, csv_file_data):
...
class CSVFileData:
    ...
    def calculate_implied_volatilities(cls, data, lines):
        trades = []
        for trade_info in data.data_as_dictionary(lines):
            trade = create_instance_of_trade_object(trade_info)
            trades.append(trade)
    ...
    def data_as_dictionary(self, break_point):
        line_count, keys, data = 0, self.header, []
        for row in self.body:
            if line_count == break_point + 1:
                break
            else:
